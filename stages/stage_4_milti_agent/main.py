...
def search_privacy_law(query: str) -> str:
    ...
    Args:
        query: Natural language query about privacy, GDPR, CCPA, or data breaches.
    """
    knowledge = [
        (
            ["data", "breach", "leak", "rò rỉ", "ro ri"],
            "Data breach response often requires prompt investigation, containment, user notice, "
            "regulator notice, evidence preservation, and remediation. Exposure may include "
            "regulatory fines, consumer claims, and contractual indemnity obligations.",
        ),
        (
            ["gdpr", "privacy", "consent", "personal", "dữ liệu", "du lieu"],
            "GDPR requires a lawful basis for processing, transparency, data minimisation, "
            "security controls, and data-subject rights. Serious violations can reach EUR 20M "
            "or 4% of global annual turnover.",
        ),
        (
            ["ccpa", "cpra", "california", "consumer"],
            "CCPA/CPRA gives California consumers notice, access, deletion, correction, opt-out, "
            "and limited private action rights for certain data breaches.",
        ),
    ]
    query_lower = query.lower()
    results = []
...
async def check_routing(state: LegalState) -> dict:
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data, privacy, GDPR, CCPA, consent, breach'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
    ...
    if any(kw in question_lower for kw in ["data", "privacy", "gdpr", "ccpa", "dữ liệu", "du lieu"]):
        needs_privacy = True
    print(
        "  [Node: check_routing] "
...
async def call_tax_specialist(state: LegalState) -> dict:
    ...
    if not final_msg:
        fallback = await llm.ainvoke([HumanMessage(content=f"{tax_prompt}\n\nQuestion: {state['question']}")])
        final_msg = fallback.content
...
async def call_compliance_specialist(state: LegalState) -> dict:
    ...
    final_msg = result["messages"][-1].content
    if not final_msg:
        fallback = await llm.ainvoke([HumanMessage(content=f"{compliance_prompt}\n\nQuestion: {state['question']}")])
        final_msg = fallback.content
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        ...
        "response, consent, data-subject rights, and security obligations. Use the "
        "search_privacy_law tool to ground your analysis. Keep your response under 200 words."
    )
    ...
    final_msg = result["messages"][-1].content
    if not final_msg:
        fallback = await llm.ainvoke([HumanMessage(content=f"{privacy_prompt}\n\nQuestion: {state['question']}")])
        final_msg = fallback.content
...
QUESTION = (
    "If a company has a customer data breach, shares user data without consent, "
    "and avoids taxes, what are the legal and regulatory consequences?"
)
